...
<head>
    ...
    <title>Check-out | HRMS Enterprise</title>
    ...
    <style>
        ...
        .parallax-bg{position:fixed;inset:0;z-index:0;background:url('https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=1920&q=80') center/cover no-repeat;background-attachment:fixed}
        ...
        .cloud-1{top:12%;width:440px;height:130px;background:radial-gradient(ellipse,#fff 0%,transparent 70%);animation:drift 40s linear infinite}
        .cloud-2{top:40%;width:500px;height:140px;background:radial-gradient(ellipse,#fff 0%,transparent 70%);animation:drift 55s linear infinite;animation-delay:-18s}
        ...
        .glass:hover{border-color:rgba(251,146,60,.4);box-shadow:0 30px 60px rgba(0,0,0,.4),0 0 80px -20px rgba(251,146,60,.15)}

        .clock{font-family:'Montserrat',sans-serif;font-size:4.5rem;font-weight:900;letter-spacing:.08em;
            background:linear-gradient(to right,#fdba74,#fb923c,#fed7aa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;
            filter:drop-shadow(0 0 20px rgba(251,146,60,.3))}
        ...
        .btn-glow::after{content:'';position:absolute;inset:-3px;border-radius:inherit;background:linear-gradient(135deg,#ea580c,#f97316,#fb923c);opacity:0;z-index:-1;filter:blur(12px);transition:opacity .4s}
...
<body class="text-white min-h-screen">
    ...
    <main class="relative z-10 flex items-center justify-center min-h-screen px-4 py-10">
        <div class="w-full max-w-md" data-aos="fade-up" data-aos-duration="900">
            <div class="glass p-10 text-center">
                <div class="w-16 h-16 mx-auto mb-5 rounded-2xl bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-2xl shadow-lg shadow-orange-500/30 rotate-[6deg]" data-aos="zoom-in" data-aos-delay="200">
                    <i class="fa-solid fa-right-from-bracket"></i>
                </div>

                <h1 class="font-heading text-3xl md:text-4xl font-extrabold mb-1 bg-gradient-to-r from-orange-300 via-amber-200 to-yellow-200 bg-clip-text text-transparent" data-aos="zoom-in" data-aos-delay="300">
                    Chấm Công Ra
                </h1>
                <p class="text-slate-400 text-sm mb-8">Ghi nhận giờ kết thúc làm việc</p>
                ...
                <form id="checkout-form" data-aos="fade-up" data-aos-delay="500">
                    <button type="submit" id="btn-checkout"
                        class="btn-glow w-full py-4 rounded-2xl font-heading font-bold text-lg
                               bg-gradient-to-r from-orange-600 to-red-500
                               hover:from-orange-500 hover:to-red-400
                               shadow-xl shadow-orange-600/25 hover:shadow-orange-500/40
                               transition-all duration-300 hover:-translate-y-1 cursor-pointer
                               flex items-center justify-center gap-3">
                        <i class="fa-solid fa-door-open text-xl"></i>
                        <span id="btn-text">Check Out Now</span>
                    </button>
                </form>

                <div class="mt-6 flex justify-center gap-4 text-sm" data-aos="fade-up" data-aos-delay="600">
                    <a href="check-in.php" class="text-orange-300 hover:text-white transition"><i class="fa-solid fa-right-to-bracket mr-1"></i>Check-in</a>
                    <span class="text-slate-600">|</span>
                    <a href="attendance-list.php" class="text-orange-300 hover:text-white transition"><i class="fa-solid fa-clock-rotate-left mr-1"></i>Lịch sử</a>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        const form=document.getElementById('checkout-form'),btn=document.getElementById('btn-checkout'),
        ...
        form.addEventListener('submit',async e=>{
            ...
            try{
                const res=await fetch('attendance_controller.php?action=checkOut',{method:'POST'});
                const json=await res.json();
                showAlert(json.success?'success':'error',json.message);
            }catch(err){showAlert('error','Lỗi kết nối máy chủ');}
            btnText.textContent='Check Out Now'; btn.disabled=false;
        });
    </script>
</body>
</html>